...
2.  **SCREENS**: Each screen object MUST have an \`id\`, a \`title\`, and a \`layout\` object. Screen \`id\`s MUST be strings containing only uppercase letters and underscores (e.g., SCREEN_NAME, WELCOME_STEP). Numbers are not allowed. The last screen should be marked with \`"terminal": true\`.
...
- **Complete/Data Exchange**: Use a \`payload\` object to gather data: \`{ "name": "complete", "payload": { "field": "\u0024{form.form_name.field_name}" } }\`
